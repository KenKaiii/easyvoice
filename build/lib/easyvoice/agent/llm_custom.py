...
class CustomLLMInterface:
    ...
    async def generate_response_stream(self, 
                                     message: str,
                                     context: Optional[List[Dict[str, str]]] = None) -> AsyncGenerator[str, None]:
    ...
    def _prepare_messages(self, 
                         message: str, 
                         context: Optional[List[Dict[str, str]]] = None) -> List[Dict[str, str]]:
        ...
        messages.append({
            "role": "system",
            "content": "You are a helpful voice assistant. Respond conversationally and concisely. Keep responses under 100 words unless more detail is specifically requested."
        })
    ...
    async def generate_with_tools(self,
        ...
        if any(word in message_lower for word in ["memory", "system", "performance"]):
            try:
                sys_result = await tools_manager.execute_tool("system_info", {"info_type": "memory"})
        ...
        if any(word in message_lower for word in ["weather", "temperature"]):
            ...
            try:
                weather_result = await tools_manager.execute_tool("weather", {"location": location})
