...
@click.pass_context
def main(ctx, version: bool, config: Optional[str]):
...
@click.pass_context
def listen(ctx, timeout: Optional[int], push_to_talk: bool, verbose: bool):
...
@click.pass_context
def ask(ctx, question: str, voice: bool, save: bool):
...
@click.pass_context
def test_audio(ctx, duration: int, verbose: bool):
    ...
    try:
        ...
        if asyncio.run(test_text_to_speech(
            text=test_text,
            settings=settings,
            verbose=verbose
        )):
