...
async def load_kitten_model(test_settings):
    ...
    with patch('easyvoice.audio.tts.KittenTTS._load_model_sync', return_value=pytest.mock_kitten_model):
...
async def synthesize_different_voices():
    ...
    for voice_id, voice_name in voices_to_test:
        ...
        unique_audio = np.random.random(24000).astype(np.float32) * 0.3 + voice_id * 0.01
        pytest.mock_kitten_model.generate.return_value = unique_audio
        
        result = await pytest.kitten_tts.synthesize_text(f"Test voice {voice_id}", voice=voice_id)
...
async def synthesize_invalid_voice():
    ...
    try:
        pytest.invalid_voice_result = await pytest.kitten_tts.synthesize_text("Test", voice=10)
...
async def have_synthesized_audio():
    ...
    pytest.playback_audio_data = await pytest.kitten_tts.synthesize_text("Test playback")
...
async def play_audio():
    ...
    with patch('sounddevice.play') as mock_play, \
        ...
        pytest.playback_result = await pytest.kitten_tts.play_audio(pytest.playback_audio_data)
