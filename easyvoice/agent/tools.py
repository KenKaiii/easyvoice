...
class Tool:
    ...
    def __init__(
        self, name: str, func: Callable, description: str = "", timeout: int = 30
    ):
        """Initialize tool
        ...
        try:
            ...
            if self.is_async:
                return await asyncio.wait_for(
                    self.func(**bound_args.arguments), timeout=self.timeout
                )
            else:
                # Run sync function in thread pool
                loop = asyncio.get_event_loop()
                return await loop.run_in_executor(
                    None, lambda: self.func(**bound_args.arguments)
                )
...
class ToolsManager:
    ...
    def _register_builtin_tools(self) -> None:
        """Register built-in tools"""
        # Time tool
        self.register_tool("time", self._get_current_time, "Get current date and time")
        ...
        self.register_tool(
            "weather", self._get_weather, "Get weather information for a location"
        )
        ...
        return [
            {
                "name": tool.name,
                "description": tool.description,
                "timeout": str(tool.timeout),
                "is_async": str(tool.is_async),
            }
    ...
    def _get_system_info(self, info_type: str = "memory") -> str:
        ...
        try:
            if info_type.lower() == "memory":
                ...
                return (
                    f"System performance - CPU: {cpu_percent}%, "
                    f"Memory: {memory.percent}%"
                )

            else:
                return (
                    f"Unknown info type: {info_type}. "
                    "Available: memory, cpu, disk, performance"
                )
    ...
    def _get_weather(self, location: str = "current location") -> str:
        ...
        if location_key in mock_weather_data:
            ...
            return (
                f"Weather information for {location}: 20°C, partly cloudy "
                "(mock data - API integration needed)"
            )


def create_tool_executor(tools_manager: ToolsManager) -> Callable:
