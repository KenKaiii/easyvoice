        ...
        if energy > self.threshold:
            ...
            if self.is_speaking:
                ...
                if (
                    ...
                    and current_time - self.speech_start_time
...
class AudioInput:
    ...
    def __init__(self, settings: Settings):
        ...
        self.vad = VoiceActivityDetector(
            threshold=settings.vad_threshold, min_speech_duration=0.5
        )
    ...
    def _validate_audio_device(self) -> None:
        ...
        try:
            sd.query_devices()  # Just validate devices are available
    ...
    async def record_until_silence(
        self, max_duration: float = 30.0, silence_duration: float = 1.0
    ) -> np.ndarray:
...
def test_microphone(duration: float = 2.0, verbose: bool = False) -> bool:
    ...
    try:
        ...
        return bool(max_amplitude > 0.001)  # Very low threshold
...
async def test_voice_activity_detection(
    settings: Settings, duration: float = 5.0, verbose: bool = False
) -> bool:
